...
class enrol_wallet_plugin extends enrol_plugin {
    ...
    public function get_instance_name($instance) {
        ...
        if (empty($instance->name)) {
            ...
            if (isset($instance->cost) && is_numeric($instance->cost) && $instance->cost >= 0) {
                $cost = $instance->cost;
                $currency = $instance->currency ?? config::make()->currency ?? '';
                $name .= "- {$cost} {$currency}";
            }
    ...
    public function enrol_self(stdClass $instance, $user = null, $charge = true) {
        ...
        $instance = $this->get_helper($instance, $user->id);
        $op = new balance_op($user->id, $instance->get_course_category());
        ...
        $cashbackenabled = config::make()->cashback;
    ...
    public function hide_due_cheaper_instance(stdClass $thisinstance): bool {
        global $DB, $USER;
        $thisinstance = $this->get_helper($thisinstance);
    ...
    public function enrol_page_hook(stdClass $instance) {
        ...
        if (true === $enrolstatus) {
            ...
            if (!empty($instance->customint1) && !empty($instance->currency)) {
                $output .= self::show_payment_info($instance);
            }
    ...
    public function can_self_enrol(stdClass $instance, $checkuserenrolment = true) {
        ...
        $repurchase = config::make()->repurchase;
        $instance = $this->get_helper($instance);
    ...
    public function edit_instance_form($instance, \MoodleQuickForm $mform, $context) {
        ...
        // Todo: Change the behaviour of startdate and enddate to be the enrollment start and end.
        // Enrol start date.
    ...
    public function edit_instance_validation($data, $files, $instance, $context) {
        ...
        $tovalidate['customint5'] = (count($cohorts) > 0) ? $validcohorts : 0;
        ...
        $offerserrors = offers::validate($data);

        $errors = array_merge($errors, $typeerrors, $offerserrors);
    ...
    public function get_instance_defaults() {
        ...
        $fields = [
            ...
            'enrolperiod'     => $config->enrolperiod,
            'expirynotify'    => $expirynotify,
            'notifyall'       => $notifyall,
            'expirythreshold' => $config->expirythreshold,
            'currency'        => $config->currency,
            'customint1'      => $config->paymentaccount,
            'customint2'      => $config->longtimenosee,
            'customint3'      => $config->maxenrolled,
            'customint4'      => $config->sendcoursewelcomemessage,
            'customint5'      => 0,
            'customint6'      => $config->newenrols,
            'customint7'      => 0,
            'customtext2'     => '',
        ];
    ...
    public static function check_discount_coupon(): ?string {
        return coupons::check_discount_coupon();
    }
