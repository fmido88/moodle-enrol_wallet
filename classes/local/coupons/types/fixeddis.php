...
class fixeddis extends base {
    ...
    public static function get_visible_name(): string {
        return get_string('fixeddiscountcoupon', 'enrol_wallet');
    }

    #[\Override()]
    public function validate_coupon(area_base $area, int $userid, ?string &$error = null): bool {
        if ($this->value <= 0) {
            $error = get_string('invalidfixeddiscountcoupon', 'enrol_wallet');
